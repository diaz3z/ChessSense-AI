...
def detect_chess_pieces(frame):
    # Use YOLOv8 model to detect chess pieces
    results = model(frame)

    # Parse the results to get bounding boxes and labels
    bboxes = results.xyxy[0][:, :4]  # Get bounding box coordinates
    labels = results.xyxy[0][:, -1]  # Get class labels
...
while True:
    ...
    if board_contour is not None:
        ...
        # Detect chess pieces
        bboxes, labels = detect_chess_pieces(frame)

        # Map detected pieces to square positions
        #square_positions = map_pieces_to_squares(bboxes, labels)

        # Update piece positions if necessary
        #updated_positions = update_piece_positions()
